...
const { frontEndUrl, lyMytedSEO, isDev, stripHtml } = useHelpers()
const { path } = useRoute()
...
const siteName = process.env.SITE_TITLE ?? "Lymyted"
...
const facebook =
  lyMytedSEO?.find((item) => item.provider === "facebook") ?? null
const twitter = lyMytedSEO?.find((item) => item.provider === "twitter") ?? null
</script>
